...
%% binning with time window 

bin = zeros(63486,185); % maximum value now is 1269712ms, with time window of 200ms
lower_bound = 0.0;
upper_bound = 20.0;
...
for x = 1:185
    for i = 1:63486 %run for every time window 
        for j = 1: length(extracted(:,x)) %0-5
            if extracted(j,x) < upper_bound && extracted(j,x) >= lower_bound
                bin(i,x) = bin(i,x) + 1;
            end

            if extracted(j,x) > upper_bound %stop when theres zero 
                 break;
            end
        end
        lower_bound = lower_bound + 20; %move to the next one 
...
%% elimiate all the non one values into one (apparently 20ms is not small enough for all ones resolution)

bin_2 = zeros(63486,185);
...
%% checking about the range

check = zeros(1,185);

for i = 1:185 
    check(1,i) = find(bin_2(:,i),1,'last');
end
